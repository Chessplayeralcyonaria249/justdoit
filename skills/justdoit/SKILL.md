---
name: justdoit
description: "Turn a raw task, feature request, PRD, or repo-scoped project brief into durable execution docs: `plans.md`, `status.md`, and `test-plan.md` (or existing repo equivalents). Use when Codex needs to break work into dependency-ordered milestones, validation steps, test gates, assumptions, blockers, and a resumable status log. Triggers include requests like \"сделай plans.md/status.md\", \"разложи задачу по milestone\", \"prepare plan and status files\", or \"turn this task into execution docs\"."
---
